...
- **Run from chat** — every code block in a reply keeps its **Run** button: runnable files (HTML/SVG/CSS/JS) open a live preview in a new tab; anything else opens as a raw file download. Copy and download buttons stay on every block
...
- **Offline test suite** — `npm test` runs 57 automated tests (driver, API, auth, UI, compaction) against a mock ChatGPT page, no real ChatGPT account or network access needed
...
- [x] Run button on chat code blocks (live preview tab for HTML/SVG/CSS/JS, raw file otherwise)
- [x] Image support (attach / paste / drag-drop, vision via ChatGPT session, persisted in history + export, Discord bot attachments)
...
- **v8.1.4** — Live streaming + Discord bot commands + resilient demo site. **Discord bot:** the gateway's answer now streams into the chat in real time instead of arriving as one block (`stream:true` SSE, throttled edits every 250ms), code blocks come back as downloadable file attachments (max 10, `Dockerfile` handled), `/chat` and `/ask` no longer require the `message:` option (a bare command shows a usage hint), new `/demo` and `/credits` commands, and `/status` now reads the gateway's real data instead of always reporting "not ready". **Demo site (pages.dev):** the stuck composer is fixed — deleting a chat, starting a new chat, or pressing Stop now aborts the in-flight stream instead of leaving the input disabled forever (client-side `AbortController`, so Stop works even when the tunnel is down); modals can no longer go off-screen (scrollable, `margin:auto` centering); code blocks get proper syntax highlighting (GitHub-style token colors, dark + light); the mic button always shows (with a hint on browsers without voice support); and the site now has a serverless OpenAI-compatible API — `/v1/models` and `/v1/chat/completions` (plain + streaming SSE) run on Workers AI whenever the tunnel/gateway at home is unreachable, so the API modal, Cursor, and any OpenAI client keep working on the demo. The GitHub button hover was also reworked (the animation moved to the SVG so the link can't be flickered into unclickability). 57 tests green (network-dependent suites may need a healthy connection).

- **v8.1.3** — GitHub button hover fix + Discord bot launch fixes + 3-second-reply fix. The GitHub button now rolls off-screen and cleanly rolls back instead of teleporting. The web UI surfaces TTS/voice errors properly (server errors, autoplay-blocked, mic permission/no-speech/network) instead of dying silently. **Discord bot:** `run.bat` no longer recurses itself into "Maximum setlocal recursion level reached" (the `&` in the title was being executed as a command separator), the gateway URL is read from `config.json` with a real JSON parse, and a parenthesis in echoed text inside an `if ( )` block no longer trips cmd's parser into "then was unexpected at this time" — the launcher gets all the way to the bot. `bot.js` tolerates newer discord.js where `setActivity`/`setUsername` return non-Promises, survives old `data.json` files that predate personas, and every slash command now acknowledges Discord within the 3-second window (`deferReply`) and edits the reply when the gateway answers (a slow gateway used to kill the process with `10062 Unknown interaction` / "application didn't respond"); private commands stay ephemeral. 57 tests green.

- **v8.1.2** — Image attachments fixed on the real ChatGPT page + auto-kill on startup. The driver now scopes the file input to the composer's own form (so a stray avatar/upload input elsewhere can't swallow your image), waits for the composer's React handler to pick the files up (re-applies them if the first attempt lands too early), and detects the thumbnail/chip the way ChatGPT actually renders it (filename alt, `data:image`/blob/estuary src, chips that name the file) — an empty `.files` list is no longer treated as a failure because real ChatGPT clears the input once it consumes them. **Auto-kill:** starting the server now kills a previous instance still bound to the same port (verified it's this gateway first) instead of dying with EADDRINUSE — applies to `node server.js`, `npm start`, the .bat launchers and the in-app update/restart flow. The mock page now mirrors the real upload lifecycle so the driver test covers the true failure mode. 57 tests green.

- **v8.1.1** — Fix image attachments (setInputFiles selector string vs array bug behind "could not attach the image to the composer"), GitHub button rolls off-screen and back instead of spinning 90deg, remove the Code tab (Run/Copy/Download stay on chat code blocks, runnables open a live preview tab).

- **v8.1** — Image support end-to-end (attach/paste/drop + vision through real ChatGPT, OpenAI `image_url` parts, Discord `/chat` `/ask` image attachments), Code tab live preview, GPT-5.6 Luna badge, pages.dev hides LAN IPs, resilient offline status.
